  ...
  details.forEach(async (order) => {
    ...
    try {
      const result = await pool
        .query(
          "Insert into payment_details (Payment_date,Project_id,Amount,Created_by,Created_datetime) values (?,?,?,?,?);",
          [Pay_Date, Project_id, Number(Amount), username, datetime]
        )
...
exports.fetchPaymentUpdate = async (details) => {
  try {
    const row = await pool
      .query(
        "select * from payment_details where Payment_date = ? and Project_id =? ;",
...
exports.updatePaymentDetails = async (details) => {
  ...
  details.forEach(async (order) => {
    ...
    try {
      await pool
        .query(
          "update payment_details set Payment_date = ?,Amount =?,Updated_by =?,Updated_datetime =? where Project_id =? and Payment_id = ?;",
          [
...
exports.projectPaymentDelete = async (Details) => {
  const row = await pool
    .query(
      "Delete from payment_details where Project_id = ? and Payment_id=?;",
      [Details.Project_id, Details.Payment_id]
    )
    .catch((err) => console.log(err));
  console.log(row);
  return "success";
...
exports.allMaterialPaymentUpdate = async (Details) => {
  try {
    const select = await pool
      .query(
        'SELECT Project_id,Material_Name,CREATED_DATETIME,Supplier_name,Balance,Amount,Paid FROM order_details WHERE STATUS <>"Paid" AND (Supplier_name = ?) AND (Order_date BETWEEN ? AND ?) Order by CREATED_DATETIME;',
        [Details.Supplier, Details.Start, Details.End]
      )
      .catch((err) => console.log(err));
    for (const item of select) {
      if (Total <= item.Balance) {
        ...
        if (item.Balance === 0) {
          await pool
            .query(
              'Update order_details set Paid=Amount,Status="Paid",Balance=0, Payment_Date =? WHERE STATUS<>"Paid" AND (Supplier_name = ?) and (Project_id=?) and (Material_Name = ?) AND (CREATED_DATETIME = ?);',
              [
          ...
          await pool
            .query(
              "insert into material_payments (Project_id,Bill_no,Material_name,Supplier_name,Payment_Date,Material_amount,Amount,Created_by,Created_Datetime) values(?,?,?,?,?,?,?,?,?)",
              [
                item.Project_id,
                Details.Billno,
                item.Material_Name,
          ...
          await pool
            .query(
              'Update order_details set Paid=?,Status="UnPaid",Balance=?, Payment_Date =? WHERE STATUS<>"Paid" AND (Supplier_name = ?) and (Project_id=?) and (Material_Name = ?) AND (CREATED_DATETIME = ?);',
              [
...
exports.deleteMaterialPayments = async (Details) => {
  try {
    const result = await pool
      .query("select * from material_payments where Bill_no =?", [
        Number(Details.Billno),
      ])
      .catch((err) => console.error(err));
    await pool
      .query("delete from material_payments where Bill_no =?", [
        Number(Details.Billno),
      ])
      .catch((err) => console.error(err));
    console.log(result);
    for (const item of result) {
      const data = await pool
        .query(
          "select Paid,Amount from order_details where Project_id =? and Material_Name =? and Supplier_Name =?",
          [item.Project_Id, item.Material_name, item.Supplier_name]
        )
        .catch((err) => console.error(err));
      // console.log("================================== >",data)
      await pool
        .query(
          'update order_details set Paid = ? , Balance =?,Status = "UnPaid" where Project_id =? and Material_Name =? and Supplier_Name =?',
          [
            Number(data[0].Paid) - Number(item.Material_amount),
...
exports.allMaterialPayment = async (Details) => {
  if (Details.Supplier == "null") {
    try {
      ...
      const material = await pool
        .query(
          'SELECT * FROM order_details WHERE STATUS <>"Paid" AND (Order_date BETWEEN ? AND ?) ;',
    ...
    try {
      ...
      const material = await pool
        .query(
          'SELECT * FROM order_details WHERE STATUS <>"Paid" AND (Supplier_name = ?) AND (Order_date BETWEEN ? AND ?);',
          [Details.Supplier, Details.Start, Details.End]
        )
        .catch((err) => console.log(err));
...
exports.fetchMaterialBalance = async (Details) => {
  if (Details.Supplier == "null") {
    try {
      ...
      const material = await pool
        .query(
          'SELECT * FROM order_details WHERE (Project_id = ?) AND (STATUS <>"Paid") AND (Order_date BETWEEN ? AND ?);',
          [Details.Id, Details.Start, Details.End]
        )
        .catch((err) => console.log(err));
    ...
    try {
      ...
      const material = await pool
        .query(
          'SELECT * FROM order_details WHERE (Project_id = ?) AND (STATUS <>"Paid") AND (Supplier_name = ?) AND (Order_date BETWEEN ? AND ?);',
          [Details.Id, Details.Supplier, Details.Start, Details.End]
        )
        .catch((err) => console.log(err));
...
exports.fetchMaterialPay = async (Details) => {
  const material = await pool
    .query(
      'SELECT supplier_name,SUM(balance) as Balance FROM order_details where STATUS <>"Paid" and balance > 0 GROUP BY supplier_name ; '
    )
    .catch((err) => console.log(err));
  console.log(material);
  return material;
